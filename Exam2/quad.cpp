...
class Hash{
    ...
    public:
        ...
        int mod(int);
        void displayHash();
        void markMTAfter(int);
        int checkMTAfter(int);
};
...
void Hash::del(int key, int c1, int c2){
    ...
    while (table[index].front() != emptyFromstart && bktsProbed < N)
    {
        if ((!(table[index].empty())) && (table[index].front() == key)){
            ...
            table[index].front() = 0;
            emptyAfterRemoval = table[index].front();
        }
        else if (table[index].empty() || table[index].front() == 0){
            if (table[index].front() == emptyAfterRemoval){
                emptyFromstart = -1;
            }
            else{
